...
class EmployeeDocumentFactory extends Factory
{
    ...
    public function definition(): array
    {
        if (empty(self::$employeeDocs)) {
            ...
            $docType = ['cv', 'copia de documento', 'otros'];
            foreach ($employees as $employee_id) {
                foreach ($docType as $docTypes) {
                    self::$employeeDocs[] = ['employee_id' => $employee_id, 'document_type' => strtolower($docTypes)];
                }
            }
        }
        ...
        $documentName = 'EmployeeDocuments/' . Str::slug($document_type . '-' . $fulltname) . '.pdf';
        $options = new Options();
